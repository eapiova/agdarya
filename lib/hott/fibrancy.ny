...
| id⟨e⟩ x : (x₀ : A.0) (x₁ : A.1) → isFibrant (A.2 x₀ x₁) ] {- We include the Martin-Lof equality so that everything will typecheck, although in practice it will turn out to always be rfl and thus reduce away. -}
...
eq.trr A P a0 a1 a2 p = case a2 of λ { rfl → p}

eq.trr2 : (A : Set) (B : Set) (P : A → B → Set) (a0 a1 : A)
...
Id_eq A0 A1 A2 a00 a01 a02 a10 a11 a12 a20 a21 a22
=
...
=
  ...
  to_fro ≔ b2 ↦
    Id_eq B0 B1 B2 (e0 to (e0 fro b0)) (e1 to (e1 fro b1))
      ...
      (e2 to_fro b2)) {- Here we can cheat a little: since we're already in HOTT, we don't need to assume an explicit witness of fibrancy for the given type A, we can just use its intrinsic one -}
...
id_pi_rtr : (A0 : Set) (A1 : Set) (A2 : Id Set A0 A1) (B0 : A0 → Set)
            (B1 : A1 → Set)
            (B2
            ...
            → rtr
                ((a0 : A0) (a1 : A1) (a2 : A2 a0 a1)
                 → B2 a2 (f0 a0) (f1 a1))
                (Id ((X Y ↦ (x : X) → Y x) : (X : Set) → (X → Set) → Set)
                   A2 B2 f0 f1)

id_pi_rtr A0 A1 A2 B0 B1 B2 f0 f1
=
  (
...
id⟨e⟩ = λ f0 f1 →
    fib_rtr
      ...
      (Id ((X Y ↦ (x : X) → Y x) : (X : Set) → (X → Set) → Set) A.2 B.2 f0
         f1) (id_pi_rtr A.0 A.1 A.2 B.0 B.1 B.2 f0 f1) }

isbisim_rtr : (A B : Set) (R S : A → B → Set)
              (e : (a : A) (b : B) → rtr (R a b) (S a b))
              (Re : isBisim A B R)
              → isBisim A B S

isbisim_rtr A B R S e Re
=
  record {
trr = λ a → Re trr a;
