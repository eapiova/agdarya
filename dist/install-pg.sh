...
echo -n "Checking for Agdarya..."

if command -v agdarya >/dev/null 2>&1
then
    echo "Found."
else
    ...
    then
        echo "Failed."
        echo
        echo "I can't find agdarya in your PATH."
        ...
        echo "https://github.com/gwaithimirdain/agdarya/issues."
        exit 1
    else
        echo "Failed."
        echo
...
echo -n "Checking whether Emacs can find Agdarya..."

EXEC_PATH_FROM_SHELL=false

if emacs -Q -batch --eval='(call-process "agdarya")' >/dev/null 2>&1
then
    echo "Found."
else
    echo "Failed."
...
EOF
    ...
    then
        ...
        echo "Failed."
        echo
        echo "Your Emacs can't find Agdarya, and I don't know how to fix it."
        echo "Please open an issue at https://github.com/gwaithimirdain/agdarya/issues."
        rm -f "$TEMPINIT"
        exit 1
    fi
fi
...
echo -n "Copying the Agdarya ProofGeneral files..."

if mkdir -p $PGDIR/agdarya
then
    :
else
    echo "Failed."
    echo
    echo "I can't create a agdarya directory in the ProofGeneral directory."
    echo "Make sure you have write/execute permissions to $PGDIR."
    exit 1
fi    

# Install the agdarya elisp files, replacing any old ones.
if [ -e agdarya.el ]
then
    rm -f $PGDIR/agdarya/*.el $PGDIR/agdarya/*.elc
    if cp -f *.el $PGDIR/agdarya
    then
        echo "Succeeded."
    else
        echo "Failed."
...
elif [ -e ../proofgeneral/agdarya.el ]
then
    echo
    echo "You appear to be running this script from the Agdarya source tree."
    echo -n "Symlinking the elisp files..."
    pushd ../proofgeneral >/dev/null
    AGDARYA_PGDIR=`pwd`
    popd >/dev/null
    pushd $PGDIR/agdarya >/dev/null
    rm -f *.el *.elc
    if ln -s $AGDARYA_PGDIR/*.el .
    then
        echo "Succeeded."
    else
        echo "Failed."
        echo -n "Trying to copy them instead..."
        if cp $AGDARYA_PGDIR/*.el .
        then
            echo "Succeeded."
        else
            echo "Failed."
            echo
            echo "Make sure you have write/execute permissions to $PGDIR/agdarya."
            echo "You can also open an issue at https://github.com/gwaithimirdain/agdarya/issues."
            exit 1
        fi
    fi
    popd >/dev/null
    ...
    echo "Failed."
    echo
    echo "I can't find agdarya.el."
    echo "Please run this script from the unpacked static distribution directory"
    echo "or from the dist/ directory in the Agdarya source tree."
    exit 1
fi

# Insert Agdarya into the ProofGeneral configuration, if it isn't already there
if grep agdarya $PGDIR/generic/proof-site.el >/dev/null
then
    echo "ProofGeneral is already configured for Agdarya."
else
    echo -n "Configuring ProofGeneral for Agdarya..."
...
echo "Agdarya ProofGeneral mode installed."
...
then
    ...
    then
        echo "Found."
        echo -n "Installing Agdarya ctags configuration..."

        if mkdir -p $HOME/.ctags.d
        then
            if [ -e agdarya.ctags ]
            then
                rm -f $HOME/.ctags.d/agdarya.ctags
                if cp -f agdarya.ctags $HOME/.ctags.d
                then
                    echo "Succeeded."
                    CTAGS=true
                else
            ...
            elif [ -e ../ctags/agdarya.ctags ]
            then
                echo
                echo "You appear to be running this script from the Agdarya source tree."
                echo -n "Symlinking the Agdarya .ctags file..."
                pushd ../ctags >/dev/null
                AGDARYA_CTAGS=`pwd`
                popd >/dev/null
                pushd $HOME/.ctags.d >/dev/null
                rm -f agdarya.ctags
                if ln -s $AGDARYA_CTAGS/agdarya.ctags .
                then
                    echo "Succeeded."
                    CTAGS=true
                else
                    ...
                    if cp $AGDARYA_CTAGS/agdarya.ctags .
                    then
                        echo "Succeeded."
                        CTAGS=true
                    else
...
then
    ...
    then
        ...
        echo "in the root directory of your Agdarya project.  You'll need to do this again"
...
echo
echo "You will need to run this script again every time Emacs, ProofGeneral, or Agdarya is updated."
