...
iscontr_idfrom : (A : Set) → (a0 : A) → isContr (Σ A (a1 ↦ Id A a0 a1))
iscontr_idfrom A a0 = (
  center ≔ (a0, refl a0),
  contract ≔ a1_a2 ↦
    let a1 ≔ a1_a2 fst in
    let a2 ≔ a1_a2 snd in
    (refl ((z ↦ Id A z a0) : A → Set) a2 trr (refl a0),
     sym (refl ((z ↦ Id A z a0) : A → Set) a2 liftr (refl a0))))

is11 : (A B : Set) → (R : A → B → Set) → Set
is11 A B R = sig (
...
is11_Id : (A0 A1 : Set) → (A2 : Id Set A0 A1) → (B0 B1 : Set) → (B2 : Id Set B0 B1) → (R0 : A0 → B0 → Set) → (re0 : is11 A0 B0 R0) → (R1 : A1 → B1 → Set) → (re1 : is11 A1 B1 R1) → (R2 : Id ((X Y ↦ (X → Y → Set)) : (X Y : Set) → Set) A2 B2 R0 R1) → (re2 : refl is11 A2 B2 R2 re0 re1) → (a0 : A0) → (a1 : A1) → (b0 : B0) → (b1 : B1) → (r0 : R0 a0 b0) → (r1 : R1 a1 b1) → is11 (A2 a0 a1) (B2 b0 b1) (a2 b2 ↦ R2 a2 b2 r0 r1)
is11_Id A0 A1 A2 B0 B1 B2 R0 re0 R1 re1 R2 re2 a0 a1 b0 b1 r0 r1 = (
  contrr ≔ a2 ↦
    let S : (y0 : B0) (y1 : B1) → R0 a0 y0 → R1 a1 y1 → Set
    ...
    (refl S p0 p1 q0 q1 trl u,
     v2 ↦
       let w
         ≔ re2 contrr a2 contract {(b0, r0)} {(b1, r1)} (v2 fst, v2 snd) in
       S⁽ᵉᵉ⁾ (sym (refl p0)) (sym (refl p1)) (sym (refl q0))
         (sym (refl q1)) {v2} {u} (sym w fst, sym w snd)
         (refl S p0 p1 q0 q1 liftl u) trl (refl u)),
  contrl ≔ b2 ↦
    let S : (x0 : A0) (x1 : A1) → R0 x0 b0 → R1 x1 b1 → Set
    ...
    (refl S p0 p1 q0 q1 trl u,
     v2 ↦
       let w
         ≔ re2 contrl b2 contract {(a0, r0)} {(a1, r1)} (v2 fst, v2 snd) in
       S⁽ᵉᵉ⁾ (sym (refl p0)) (sym (refl p1)) (sym (refl q0))
         (sym (refl q1)) {v2} {u} (sym w fst, sym w snd)
         (refl S p0 p1 q0 q1 liftl u) trl (refl u)))

{- Any 1-1 correspondence is a bisimulation, because 1-1 correspondences lift to identity types. -}
bisim_of_11 : (A B : Set) → (R : A → B → Set) → (re : is11 A B R) → isBisim A B R
bisim_of_11 A B R re = record {
  ...
  id⟨e⟩ = λ a0 b0 r0 a1 b1 r1 →
    bisim_of_11 (A.2 a0 a1) (B.2 b0 b1) (a2 b2 ↦ (R.2 a2 b2 r0 r1))
      (is11_Id A.0 A.1 A.2 B.0 B.1 B.2 R.0 re⟨0⟩ R.1 re⟨1⟩ R.2 re⟨2⟩ a0 a1
         b0 b1 r0 r1) }

univalence : (A B : Set) → (f : A → B) → (fe : (b : B) → isContr (Σ A (a ↦ Id B (f a) b))) → Id Set A B
univalence A B f fe = glue A B (a b ↦ Id B (f a) b)
