...
The Agdarya executable accepts at least the following command-line flags.
...
When the Agdarya executable is run, it loads all the files given on its command line and any strings supplied on the command line with ``-e``.  As usual, the special filename ``-`` refers to standard input.  Files and strings are loaded in the order they are given on the command line; all files must have the extension ``.ny``.  Lastly, if ``-i`` was given anywhere on the command line, Agdarya enters interactive mode.

In interactive mode, commands typed by the user are executed as they are entered.  Since many commands span multiple lines, Agdarya waits for a blank line before parsing and executing the command(s) being entered.  Make sure to enter a blank line before starting a new command; interactive commands must be entered and executed one at a time.  The result of the command is printed (more verbosely than is usual when loading a file) and then the user can enter more commands.  Set Control+D to exit interactive mode, or enter the command ``quit``.  In addition, in interactive mode you can enter a term instead of a command, and Agdarya will assume you mean to :ref:`echo<Echo/Synth>` it.
...
.. code-block:: none
   ...
   postulate NAME [PARAMS] : TYPE

Assert a global constant called ``NAME`` having type ``TYPE``, without any definition (an postulate).  Parameters and names are treated as for ``def``.
...
Normalize ``TERM`` and print its value and its type to standard output.  Note that ``TERM`` must synthesize a type (see :ref:`Bidirectionality`); if it is a checking term you must ascribe it.  In interactive mode, if you enter a term instead of a command, Agdarya assumes you mean to ``echo`` that term.
...
`ProofGeneral <https://proofgeneral.github.io/>`_ is a generic development environment designed for proof assistants that runs inside the text editor Emacs.  Proof General is perhaps best known for its use with `Rocq <https://rocq-prover.org/>`_.  Agdarya comes with a basic ProofGeneral mode.  Agdarya does not yet have a true interactive *proof* mode, which ProofGeneral is designed for, but it is still useful for progressive processing of commands in a file.  In addition, the Agdarya ProofGeneral mode is enhanced with commands for creating, inspecting, and filling holes, similar to Agda's Emacs mode.
...
Once Agdarya's ProofGeneral mode is installed either :ref:`automatically<Automatic ProofGeneral installation>` or :ref:`manually<Manual ProofGeneral installation>`, it should start automatically when you open a file with the ``.ny`` extension.  When ProofGeneral mode is active, there is some initial segment of the buffer (which starts out empty) that has been processed (sent to Agdarya) and is highlighted with a background color (usually blue).  The unprocessed part of the buffer can be freely edited, and as you complete new commands you can process them as well one by one.  You can also undo or "retract" processed commands, removing them from the processed region.  If you edit any part of the processed region (except for editing inside an existing comment, or :ref:`filling a hole<solving holes>` with ``C-c C-SPC``), it will automatically be retracted (using Agdarya's ``undo`` command) up to the point where you are editing.

In addition to the main window displaying your source file, there will normally be two other windows in split-screen labeled "goals" and "response" (although this can be customized with the Emacs variables ``proof-three-window-enable`` and ``proof-three-window-mode-policy``).  The "response" window displays Agdarya's informational and error messages.  The "goals" window displays the contexts and types of holes whenever relevant.

Key commands
^^^^^^^^^^^^

The most useful ProofGeneral key commands for Agdarya are the following.

- ``C-c C-n`` : Process the next unprocessed command.  Since Agdarya has no command-terminating string, the "next command" is interpreted as continuing until the following command keyword or until the end of the buffer.  This means that if you've written a complete command but there is garbage following it, in order to process the command you'll need to either comment out the garbage or insert at least the beginning of another command in between (such as ``quit``) so that ProofGeneral can find the end of the command you want to process.
...
- ``C-c C-c`` : Interrupt Agdarya if a command is taking too long.  Agdarya attempts to recover, but its state may be unreliable afterwards.
- ``C-c C-x`` : Retract the buffer and kill the Agdarya subprocess.
...
As noted above, Agdarya's ProofGeneral mode is enhanced to deal with open holes (see :ref:`Interactive proof`).  Whenever a hole is created by processing a command, the location of the hole is highlighted in ``agdarya-hole-face`` (which you can customize).  These highlights are removed when hole-creating commands are retracted.

Agdarya's ProofGeneral mode also defines the following additional key commands.
...
Agda users should beware: while a few of Agdarya's key commands are chosen to match those of Agda, many of the key sequences used by Agda have already been defined in ProofGeneral to mean something else (notable examples are ``C-c C-n`` and ``C-c C-b`` and ``C-c C-.``), leading Agdarya to choose different ones.

Here are the key commands that are basically the same in Agdarya and Agda:
...
And here is a mapping of Agda keybindings to approximately comparable Agdarya ones.
...
- Instead of ``C-c C-x C-q``, use ``C-c C-x`` (quit Agdarya subprocess).
...
And Agdarya defines some of its own faces as well.

- ``agdarya-hole-face``: the background highlight of open holes.


Ctags
-----

If you followed the instructions for :ref:`Installing Ctags`, then you should be able to use Emacs's built-in commands for finding and jumping to definitions in and between Agdarya source files.  The full documentation can be found in the `Emacs manual <https://www.gnu.org/software/emacs/manual/html_node/emacs/Find-Identifiers.html>`_, but here is a summary of the most useful commands:
...
Processing an entire file in ProofGeneral does not have *exactly* the same reformatting effect as running Agdarya on it from the command line.  They should reformat individual commands in the same way, but the command-line reformatter also ensures that distinct commands are separated by single blank lines (suitably interpreted in the presence of comments).  ProofGeneral can't do this, as it doesn't even pass blank lines and comments between commands to the Agdarya subprocess.  However, most people already separate their commands by single blank lines, so this difference is not usually a serious issue.  If a file has been formatted by the command-line reformatter, processing it in Proof General should not *change* that formatting (if it does, please report a bug).
...
It is possible to turn off the code formatter.  Unsetting the Emacs customization variable ``agdarya-reformat-commands`` will turn off reformatting in ProofGeneral, and the command-line option ``-no-format`` will turn off reformatting of input files.  However, if you don't like the way Agdarya reformats your code, I would appreciate it if you give me feedback about this rather than (or, at least, in addition to) turning it off entirely.  If ``agdarya-reformat-commands`` is turned off, you can manually reformat a command in the processed region with ``C-M-q``.  (Reformatting unprocessed commands would be too error-prone, as noted above: Agdarya wouldn't be able to tell which notations are in scope.)


jsAgdarya
-------

jsAgdarya is a JavaScript version of Agdarya that runs in a browser.  Its functionality is limited to the equivalent of ``agdarya -e "STARTUP" -i``: you can specify a single startup "file" by copying and pasting it into a text box, and then you drop into interactive mode.  Also there is no real Unicode input-mode, although there is a palette of buttons that can be used to enter a number of common Unicode characters.  These limitations are not intrinsic; we just have not yet found or implemented an appropriate frontend for anything more complicated.
