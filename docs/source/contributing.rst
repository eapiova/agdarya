Contributing to Agdarya
=====================

Contributions to Agdarya are welcome!  If you have code you want to contribute, please fork the `GitHub repository <https://github.com/gwaithimirdain/agdarya>`_ and create a pull request.  If you aren't sure whether some contribution would be accepted, you may want to open a GitHub issue first to discuss it.

Please format OCaml code using ``ocamlformat`` with the options in the supplied ``.ocamlformat`` file.  Please format Agdarya code (such as cram tests in the ``test/`` directory) using Agdarya's built-in formatter.  (Not all the existing tests are currently formatted that way, as some of them predate the formatter.)  Please format Emacs Lisp code (for the ProofGeneral mode) using Emacs' indentation functions such as ``C-M-q``, with ``indent-tabs-mode`` *off*.
...
Agdarya is licensed under the GPL v3.0.  When submitting a pull request, you retain the copyright to your code, but you consent to its distribution in perpetuity under this license or any other open-source license that may later be adopted by the maintainers.
